...
(function () {
  ...
  var BEACHFRONT_THRESHOLD = 0.25; // miles — adjust here if needed
  ...
  async function geocodeAddress(address) {
    ...
    var url     = NOMINATIM_URL + '?format=json&q=' + encoded + '&limit=1&countrycodes=us';
    try {
  ...
  async function loadBeachAccessLocations(rentalLat, rentalLon, areaZone, services) {
    ...
    var query = sb.from('beach_access_locations')
      .select('id,name,address,zone,county,latitude,longitude,services_offered')
      .eq('active', true);
